...
os.environ['TF_CPP_MIN_LOG_LEVEL'] = '10'  # maybe we can remove this line
...
# Batch size X number of iterations = number of training example shown to the NN.
# Here we need to find the best batch size to fit the best to out final dataset.
# The author have 3 different persons. For each person, 50 images- so he chose batch size of 5.
# The batch size is a hyper-parameter that defines the number of samples to work through before updating
...
# We are doing so, for example, in order to pull some features from the images so we need the pixels in normalize form.
# "validation_split" parameter means that 80% of the dataset goes to training and 20% goes to validation/ testing.
data_generator = tf.keras.preprocessing.image.ImageDataGenerator(
    rescale=1. / 255,
    validation_split=0.2)

# We separate out data set into Training, Validation & Testing.
...
# Create the base model from the pre-trained model MobileNet V2 Hidden and output layer is in the model.
# The first layer does a redundant work of classification of features which is not required to be trained
...
model = tf.keras.Sequential([
    ...
    tf.keras.layers.Dropout(0.1),  # 3
    tf.keras.layers.GlobalAveragePooling2D(),  # 4
    tf.keras.layers.Dense(30, activation='softmax')  # 5
])
...
alongside the training of the classifier you added. 
The training process will force the weights to be tuned from generic features maps to features associated specifically
to our dataset.
...
# My changes!
# Saving the model structure in JSON file and the model weights in HDF5 file.
...
* Using a pre-trained model for feature extraction: 
 When working with a small dataset, it is common to take advantage of features learned by a model trained 
 on a larger dataset in the same domain. This is done by instantiating the pre-trained model and adding a
 fully-connected classifier on top. The pre-trained model is "frozen" and only the weights of the classifier 
 get updated during training. In this case, the convolutional base extracted all the features associated with 
 each image and you just trained a classifier that determines the image class given that set of extracted features.
* Fine-tuning a pre-trained model: To further improve performance, one might want to repurpose the top-level 
 layers of the pre-trained models to the new dataset via fine-tuning.
 In this case, you tuned your weights such that your model learned high-level features specific to the dataset.
 This technique is usually recommended when the training dataset is large and very similar to the orginial dataset
